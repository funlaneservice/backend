...
export const openapiDocument = {
    ...
    schemas: {
      ...
      QuoteOptionView: {
        ...
        properties: {
          ...
          duffelOfferId: { type: "string", nullable: true, description: "Set if this option was created from a Duffel flight search result" },
          createdAt: { type: "string", format: "date-time" },
        },
        required: ["id", "label", "airline", "price", "departureTime", "details", "duffelOfferId", "createdAt"],
      },
      TravelRequestView: {
        type: "object",
        properties: {
      ...
      FlightOfferSummary: {
        type: "object",
        properties: {
          offerId: { type: "string", description: "Pass this to POST /requests/{id}/options/from-offer" },
          airline: { type: "string" },
          price: { type: "number", description: "In the offer's own currency, major units (not kobo)" },
          currency: { type: "string", description: "ISO currency code, e.g. NGN" },
      ...
      FlightSearchResponse: {
        type: "object",
        properties: { offers: { type: "array", items: ref("FlightOfferSummary") } },
        required: ["offers"],
      },
      AddQuoteOptionFromOfferRequest: toSchema(addQuoteOptionFromOfferSchema),
  ...
  paths: {
    ...
    "/requests/{id}/search-flights": {
      get: {
        tags: ["Requests"],
        summary: "Search real flight offers for a request",
        description:
          "Requires AGENT or ADMIN. Resolves the request's origin/destination to airport codes and searches Duffel for live offers matching the request's dates and budget tier. Nothing is persisted — offers must be converted via POST /requests/{id}/options/from-offer before they expire (~20-30 minutes).",
        security: [{ bearerAuth: [] }],
        parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
        responses: {
          "200": { description: "Live flight offers, sorted by price ascending", ...jsonContent(ref("FlightSearchResponse")) },
          "401": { description: "Missing, invalid, or expired token", ...jsonContent(ref("ErrorResponse")) },
          "403": { description: "Authenticated user is not an agent or admin", ...jsonContent(ref("ErrorResponse")) },
          "404": { description: "Request not found", ...jsonContent(ref("ErrorResponse")) },
          "422": { description: "Origin/destination could not be resolved to an airport", ...jsonContent(ref("ErrorResponse")) },
          "500": responses.serverError,
        },
      },
    },
    "/requests/{id}/options/from-offer": {
      post: {
        tags: ["Requests"],
        summary: "Add a quote option from a searched Duffel offer",
        description:
          "Requires the AGENT currently assigned to this request. Re-validates the offer with Duffel before creating the quote option, since offers expire quickly. Only offers priced in NGN are supported. Only allowed while the request is PENDING or OPTIONS_SENT.",
        security: [{ bearerAuth: [] }],
        parameters: [{ name: "id", in: "path", required: true, schema: { type: "string", format: "uuid" } }],
        requestBody: { required: true, ...jsonContent(ref("AddQuoteOptionFromOfferRequest")) },
        responses: {
          "201": { description: "Quote option created from the offer", ...jsonContent(ref("QuoteOptionResponse")) },
          "400": responses.validation,
          "401": { description: "Missing, invalid, or expired token", ...jsonContent(ref("ErrorResponse")) },
          "403": { description: "Caller is not the agent assigned to this request", ...jsonContent(ref("ErrorResponse")) },
          "404": { description: "Request not found", ...jsonContent(ref("ErrorResponse")) },
          "409": { description: "Request is not in an editable status, or the offer has expired", ...jsonContent(ref("ErrorResponse")) },
          "422": { description: "Offer is not priced in NGN, or has no flight segments", ...jsonContent(ref("ErrorResponse")) },
          "500": responses.serverError,
        },
      },
    },
